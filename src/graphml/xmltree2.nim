...
proc add*(result: var string, n: XmlNode, indent = 0, indWidth = 2,
          addNewLines = true; forceNoSpace:bool = false) =
  ...
  case n.k
  of xnElement:
    # initially indents and adds new lines as per function call
    if indent > 0 and not forceNoSpace:
    ...
    var avoidIndent = false
    for i in 0 ..< n.len:
      if n[i].kind in {xnText, xnEntity}:
        avoidIndent = true  
        result.add(n[i], 0, indWidth, false)
      else:
        if avoidIndent:
          result.add(n[i], indent + indWidth, indWidth, addNewLines, true) # avoids indent and newline
        else:
